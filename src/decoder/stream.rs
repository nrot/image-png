...
mod tests {
    ...
    use crate::data_sets::{for_image_gamma, for_image_source_chromaticities};
    use std::fs::File;
    use super::CHUNCK_BUFFER_SIZE;
    ...
    fn image_source_chromaticities() -> Result<(), ()> {
        ...
        for _ in for_image_source_chromaticities().iter().map(|x|trial(x.path, x.expected)){};
        Ok(())
    }

    #[tokio::test]
    async fn async_image_gamma() -> Result<(), ()> {
