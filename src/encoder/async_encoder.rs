...
pub struct Encoder<'a, W: AsyncWrite> {
    w: W,
    info: Info<'a>,
    options: Options,
}
...
impl<'a, W: AsyncWrite> Encoder<'a, W> {
...
impl<W: AsyncWrite> Drop for Writer<W> {
    fn drop(&mut self) {
        let _ = self.write_chunk(chunk::IEND, &[]);
    }
}

enum ChunkOutput<'a, W: AsyncWrite> {
    Borrowed(&'a mut Writer<W>),
    Owned(Writer<W>),
}

// opted for deref for practical reasons
impl<'a, W: AsyncWrite> Deref for ChunkOutput<'a, W> {
...
impl<'a, W: AsyncWrite> DerefMut for ChunkOutput<'a, W> {
...
struct ChunkWriter<'a, W: AsyncWrite> {
...
impl<'a, W: AsyncWrite> ChunkWriter<'a, W> {
...
impl<W: AsyncWrite> Drop for ChunkWriter<'_, W> {
    fn drop(&mut self) {
        let _ = self.flush();
    }
}
...
impl<'a, W: AsyncWrite> Wrapper<'a, W> {
...
pub struct StreamWriter<'a, W: AsyncWrite> {
...
impl<'a, W: AsyncWrite> StreamWriter<'a, W> {
...
impl<'a, W: AsyncWrite> AsyncWriteExt for StreamWriter<'a, W> {
...
impl<W: AsyncWrite> Drop for StreamWriter<'_, W> {
    fn drop(&mut self) {
        let _ = self.flush();
    }
}
